...
/**
 * Creates a flexible regex that matches the domain (http/https) OR the relative path.
 */
const createAffiliatePattern = (domainPath: string, typePattern: string, casinoPattern: string) => {
    const pathPattern = `${domainPath.replace(/\//g, '\\/')}(${typePattern})-(${casinoPattern})`;
...
export const siteConfigs: Record<SiteName, SiteConfig> = {
    ...
    supercazino: {
        ...
        affiliateUrlPattern: createAffiliatePattern('/goaffcas/', SUPERCAZINO_TYPES, SLUG_PATTERN),
        skippedPaths: [], // No skips initially
    },
    ...
    jocpacanele: {
        ...
        affiliateUrlPattern: createAffiliatePattern('/goaffcas/', JOCPACANELE_TYPES, SLUG_PATTERN),
        skippedPaths: [],
    },
    ...
    jocuricazinouri: {
        ...
        affiliateUrlPattern: createAffiliatePattern('/aff/so-', JOCURICAZINOURI_TYPES, SLUG_PATTERN),
        skippedPaths: [],
    },
    ...
    jocsloturi: {
        ...
        affiliateUrlPattern: createAffiliatePattern('/aff/', BETURI_SLOTURI_TYPES, SLUG_PATTERN),
        skippedPaths: [],
    },
    ...
    'casino.com.ro': {
        ...
        highTrafficPaths: [
            ...
            //'/new-no-deposit/', 
            //'/bonusuri-fara-depunere-online-2025/',
            '/rotiri-fara-depunere-online-2025/', 
            '/bonusuri-fara-depunere-pmax/', 
            //'/bonus-fara-depunere/',
            '/bonus-bun-venit-online-2025/', 
            '/cazinouri-noi-licentiate/', 
            '/sm-bonus-fara-depunere-casino-online/',
            '/m-oferte-cu-depunere-premium/', 
            '/bonus-fara-rulaj/', 
            '/g-oferte-fara-depunere-premium/',
            '/slot/shining-crown/', 
            '/m-oferte-fara-depunere-premium/', 
            '/cazinou/king-casino/',
            '/diverse/cazinouri-cu-mize-mici/', 
            //'/rotiri-gratuite/',
            '/ghid/bonus-elite-slots/',
            '/jocuri/pacanele-cu-speciale/', 
            '/sloturi/', 
            '/slot/sizzling-hot-deluxe/',
            '/bonus-de-bun-venit/', 
            '/cazinou/prima-casino/', 
            '/slot/40-shining-crown-bell-link-demo/',
            '/cazinou/12xbet/', 
            '/bonus-casino-standard/rotiri-gratuite-million/',
        ],
        affiliateUrlPattern: new RegExp(`^\/offer\/(${CASINO_COM_RO_TYPES})-([a-zA-Z0-9x-]+)${URL_END}`, RE_FLAGS),
        skippedPaths: [
    ...
    beturi: {
        ...
        affiliateUrlPattern: new RegExp(`^\/aff\/(${BETURI_SLOTURI_TYPES})-(${SLUG_PATTERN})${URL_END}`, RE_FLAGS),
        skippedPaths: [],
    },
} as const;
